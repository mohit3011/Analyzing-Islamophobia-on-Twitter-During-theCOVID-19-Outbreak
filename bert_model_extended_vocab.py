...
def create_new_token_list():

    keyword_list = ['stoptargetingmuslims', 'tablighiheroes', 'muslimlivesmatter', 'muslimsaviours', 
    'bringbacktablighiheroes', 'stopcovidislamophobia', 'standupformuslims', 'indiamaltreatingmuslims', 
    'hindumuslimunity', 'tablighijamatvirus', 'nizamuddinidiots', 'islamiccoronajehad', 'nizamuddinfiasco', 
    'coronajihadi', 'jihadivirus', 'tablighivirus', 'biojihad', 'nizamuddinterrorists', 'coronajihadis', 
    'islamicvirus', 'muslimsspreadingcorona', 'islamophobia', 'coronajihad']
...
class Bert_Text(nn.Module):
    ...
    def __init__(self, transformer_model, num_labels, vocab_size, config=None, device=torch.device("cuda:0")):
        super(Bert_Text, self).__init__()
        self.bert_text = transformer_model
...
#         ocr_embedding_layer = self.bert_ocr(ocr_input_ids_in, attention_mask=ocr_input_masks_in)[0]
        text_cls_token = text_embedding_layer[:,0,:]
#         ocr_cls_token = ocr_embedding_layer[:,0,:]

#         final_cls_token = torch.cat((text_cls_token, ocr_cls_token), 1)
...
def train_loop(dataloaders, dataset_sizes, transformer_model, num_classes, vocab_size, config=None, epochs=1):
    
    model = Bert_Text(transformer_model=transformer_model, num_labels=num_classes, vocab_size=vocab_size, config=config)
    
    criterion = torch.nn.NLLLoss(weight=class_weights_labels)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-6, eps=1e-08) # clipnorm=1.0, add later
...
test_label_arr = []
label_list = ['0','1']
new_token_list = create_new_token_list()
...
#------------------------------------------------------------------------------------------------
text_data, labels = prepare_dataset(datafile, data_col, label_col)

print("Number of Examples: ", len(text_data))

encoder = LabelEncoder()
encoder.fit(labels)
temp_encoded_labels = encoder.transform(label_list)
...
for train_index, test_index in skf.split(new_data, encoded_labels):
    ...
    model = train_loop(dataloaders, dataset_sizes, transformer_model, num_classes, len(tokenizer), config=config, epochs=20)
...
for i in range(len(label_list)):
    print(str(label_list[i]) + " : " + str(temp_encoded_labels[i]))